...
class EntryService:
    ...
    def entry_update(self, entry_id: int, qunatitiy: int) -> None:
        ...
        if entry.cart.user.id != self.user.id:
            raise PermissionDenied({"detail": _("You are not allowed to edit this entry")})
    ...
    def entry_delete(self, *, entry_id: int) -> None:
        entry = self.entry_get(pk=entry_id)
        if entry.cart.user.id != self.user.id:
            raise PermissionDenied({"detail": _("You are not allowed to delete this entry")})
        self.cart.count = self.cart.count - entry.quantity
        self.cart.total = self.cart.total - (entry.product.price_points * entry.quantity)
        entry.delete()
        self.cart.save()
...
def checkout(*, user: UserModel) -> dict:
    ...
    for item in cart.items.all():
        coupons.update({f"{item.product.product_name}": item.product.coupon, "link": item.product.product_page_link,})
    ...
    if offer:
        ...
        if user_offer:
            ...
            if points >= offer.required_points:
                ...
                UserOffer.objects.create(user=user, offer=offer, remaining_amount=remaining)
    return 0
